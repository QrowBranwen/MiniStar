...
class StarCraft2Env(MultiAgentEnv):
    ...
    def __init__(
        ...
        self.new_unit_positions = np.zeros((self.n_agents + self.n_enemies, 2))
        self.previous_ally_units = None
        self.previous_enemy_units = None
        self.last_action = np.zeros((self.n_agents * (1 + self.camp), self.n_actions))
    ...
    def _launch(self):
        ...
        interface_options = [
            sc_pb.InterfaceOptions(raw=True, score=False)
        ...
        self._sc2_procs = [
            ...
            for interface in interface_options
        ]
        ...
        joins = [
            sc_pb.RequestJoinGame(
                race=races[self._agent_race], options=interface
            )
            for interface in interface_options
        ]
        
        self._ports = portspicker.pick_unused_ports(self._num_agents * 2)
        for a_id in range(self._num_agents):
            joins[a_id].shared_port = 0  # unused
            joins[a_id].server_ports.game_port = self._ports[0]
            joins[a_id].server_ports.base_port = self._ports[1]
            joins[a_id].client_ports.add(game_port=self._ports[2],
                                base_port=self._ports[3])
        
        self._parallel.run((c.join_game, join)
                       for c, join in zip(self._controllers, joins))            

        self._game_info = self._parallel.run(
            c.game_info for c in self._controllers
        )

        for g, interface in zip(self._game_info, interface_options):
            if g.options.render != interface.render:
                logging.warning(
                    "Actual interface options don't match requested options:\n"
                    "Requested:\n%s\n\nActual:\n%s", interface, g.options)

        map_info = self._game_info[0].start_raw
    ...
    def reset(self, episode_config={}):
        ...
        try:
            ...
            self._obs = self._parallel.run(
                c.observe for c in self._controllers
            )
            self.init_units(
        ...
        if self.camp == 0:
            ...
            global_state_1 = np.array([self.get_state_agent(agent_id) for agent_id in range(self.n_agents)])
            global_state_2 = np.array([self.get_state_agent(agent_id, camp=1) for agent_id in range(self.n_enemies)])
            global_state = np.concatenate([global_state_1,global_state_2])
        # self._test_obs()
        return self.get_obs(camp=1), global_state # self.get_state()

    def _test_obs(self):
        obs1 = np.array([self.get_state_agent(agent_id) for agent_id in range(self.n_agents)])
        obs2 = np.array([self.get_state_agent(agent_id, camp=1) for agent_id in range(self.n_enemies)])
        p_obs = self.get_obs(camp=1)
        self._parallel.run(
            (c.step, 2) for c in self._controllers
        )
        self._kill_all_units()
        ally_team = self.episode_config.get("team_gen", {}).get("ally_team", None)
        enemy_team = self.episode_config.get("team_gen", {}).get("enemy_team", None)
        self.ally_start_positions = self.episode_config.get(
            "enemy_start_positions", {}
        ).get("item", None)
        self.enemy_start_positions = self.episode_config.get(
            "ally_start_positions", {}
        ).get("item", None)
        self.init_units(
                ally_team, enemy_team, episode_config=self.episode_config
            )
        _obs1 = np.array([self.get_state_agent(agent_id) for agent_id in range(self.n_agents)])
        _obs2 = np.array([self.get_state_agent(agent_id, camp=1) for agent_id in range(self.n_enemies)])
        _p_obs = self.get_obs(camp=1)
    ...
    def full_restart(self):
        ...
        # while not is_start:
        #     try:
        print(f'\n *******************Start full_restart!!!, force restats cnt is {self.force_restarts}*********************')
        self.close()
        sleep(random.random())
        self._launch()
        sleep(random.random())
        self.force_restarts += 1
        print(f'\n *******************Finished full_restart!!!, force restats cnt is {self.force_restarts}*********************')
            # except (protocol.ProtocolError, protocol.ConnectionError):
    ...
    def step(self, actions):
        ...
        for a_id, action in enumerate(actions_int[:self.n_agents]):
            ...
            if sc_action:
                sc_actions_1.append(sc_action)
        for a_id, action in enumerate(actions_int[self.n_agents:]):
            if not self.heuristic_ai:
                sc_action = self.get_agent_action(a_id, action, camp=1)
            else:
                sc_action, action_num = self.get_agent_action_heuristic(
                    a_id, action
                )
                actions[a_id] = action_num
            if sc_action:
                sc_actions_2.append(sc_action)
        # Send action request
        req_actions_1 = sc_pb.RequestAction(actions=sc_actions_1)
        req_actions_2 = sc_pb.RequestAction(actions=sc_actions_2)
        req_actions = [req_actions_1, req_actions_2]
        ...
        try:
            ...
            self._parallel.run(
                (c.actions, a)
                for c, a in zip(self._controllers, req_actions)
            )
        ...
        terminated = False
        reward_ally, reward_enemy = self.reward_battle()
        ...
        if game_end_code is not None:
            ...
            if game_end_code == 1 and not self.win_counted:
                ...
                if not self.reward_sparse:
                    reward_ally += self.reward_win
                    reward_enemy -= self.reward_win if not self.reward_only_positive else 0
                else:
                    reward_ally = 1
                    reward_enemy = -1
                ...
                if not self.reward_sparse:
                    reward_ally -= self.reward_win if not self.reward_only_positive else 0
        ...
        if self.debug:
            logging.debug("Reward = {}".format(reward_ally).center(60, "-"))

        if terminated:
            self._episode_count += 1

        if self.reward_scale:
            reward_ally /= self.max_reward / self.reward_scale_rate
            reward_enemy /= self.max_reward_2 / self.reward_scale_rate

        self.reward = reward_ally
        self.reward_enemy = reward_enemy

        return reward_ally, reward_enemy, terminated, info

    def get_agent_action(self, a_id, action, camp=0):
        ...
        unit = self.get_unit_by_id(a_id, camp=camp)
        tag = unit.tag
        x = unit.pos.x
        y = unit.pos.y
        ...
        if action == 0:
            ...
            if (
                ...
                target_unit = self.agents[target_id] if camp == 0 else self.enemies[target_id]
                action_name = "heal"
            else:
                target_unit = self.enemies[target_id] if camp == 0 else self.agents[target_id]
                action_name = "attack"
    ...
    def reward_battle(self):
            ...
            if not self.death_tracker_ally[al_id]:
                ...
                if al_unit.health == 0:
                    ...
                    if not self.reward_only_positive:
                        delta_ally_deaths += self.reward_death_value * neg_scale
        ...
        if self.reward_only_positive:
            reward_ally = max(delta_enemy + delta_enemy_deaths - delta_ally_deaths, 0)  # shield regeneration
            reward_enemy = max(delta_ally - delta_enemy_deaths + delta_ally_deaths, 0)  # shield regeneration
        else:
            reward_ally = delta_enemy + delta_enemy_deaths - delta_ally_deaths - delta_ally
            reward_enemy = delta_ally - delta_enemy_deaths + delta_ally_deaths - delta_enemy


        return reward_ally, reward_enemy
    ...
    def unit_shoot_range(self, agent_id, camp=0):
        ...
        if self.use_unit_ranges:
            ...
            unit = self.agents[agent_id] if camp == 0 else self.enemies[agent_id]
            return max(attack_range_map[unit.unit_type], self.min_attack_range)
        else:
            return 6

    def unit_sight_range(self, agent_id, camp=0):
    ...
    def is_position_in_cone(self, agent_id, pos, range="sight_range", camp=0):
        ally_pos = self.get_unit_by_id(agent_id, camp=camp).pos
        ...
        if range == "sight_range":
            unit_range = self.unit_sight_range(agent_id, camp=camp)
        elif range == "shoot_range":
            unit_range = self.unit_shoot_range(agent_id, camp=camp)
        else:
    ...
    def get_obs_agent(self, agent_id, fully_observable=False, camp=0):
        ...
        """
        unit = self.get_unit_by_id(agent_id, camp=camp)
        ...
        if (
            ...
            x = unit.pos.x
            y = unit.pos.y
            sight_range = self.unit_sight_range(agent_id, camp=camp)
            ...
            player1 = self.enemies if camp == 0 else self.agents
            player2 = self.agents if camp == 0 else self.enemies
            # Enemy features
            for e_id, e_unit in player1.items():
                e_x = e_unit.pos.x
                e_y = e_unit.pos.y
                dist = self.distance(x, y, e_x, e_y)
                enemy_visible = (
                    self.is_position_in_cone(agent_id, e_unit.pos, camp=camp)
                    if self.conic_fov
            ...
            for i, al_id in enumerate(al_ids):
                ...
                al_unit = self.get_unit_by_id(al_id, camp=camp)
                al_x = al_unit.pos.x
                al_y = al_unit.pos.y
                dist = self.distance(x, y, al_x, al_y)
                ally_visible = (
                    self.is_position_in_cone(agent_id, al_unit.pos, camp=camp)
                    if self.conic_fov
                    else dist < sight_range
                )
                if (ally_visible and al_unit.health > 0) or (
                    ...
                    if self.unit_type_bits > 0 and (
                        ...
                        ind += self.unit_type_bits
                    if self.obs_last_action:
                        ally_feats[i, ind:] = self.last_action[al_id + (camp * self.n_agents)]
    ...
    def get_state_agent(self, agent_id, camp=0):
        """Returns the global state.
        NOTE: This function should not be used during decentralised execution.
        """
        if self.obs_instead_of_state:
            obs_concat = np.concatenate(self.get_obs(camp=camp), axis=0).astype(
                np.float32
            )
            return obs_concat
        ...
        if unit.health > 0:  # otherwise dead, return all zeros
            ...
            for e_id, e_unit in player1.items():
                ...
                if e_unit.health > 0 or (
                    e_unit.health > 0 and self.fully_observable
                ):  # visible and alive
                    # Sight range > shoot range
                    enemy_feats[e_id, 0] = avail_actions[
                        self.n_actions_no_attack + e_id
    ...
    def get_obs(self, camp=0):
        ...
        if camp == 1:
            agents_obs_2 = [
            self.get_obs_agent(i, fully_observable=self.fully_observable, camp=camp)
            for i in range(self.n_enemies)
            ]
            agents_obs = agents_obs + agents_obs_2
        return agents_obs
    ...
    def get_avail_agent_actions(self, agent_id, camp=0):
        ...
        if unit.health > 0:
            ...
            shoot_range = self.unit_shoot_range(agent_id, camp=camp)

            target_items = self.enemies.items() if camp == 0 else self.agents.items()
            if (
                self.map_type in ("MMM", "terran_gen")
                and unit.unit_type == self.medivac_id
            ):
                # Medivacs cannot heal themselves or other flying units
                agents = self.agents if camp == 0 else self.enemies
                target_items = [
                    (t_id, t_unit)
                    for (t_id, t_unit) in agents.items()
                ...
                if t_unit.health > 0:
                    ...
                    can_shoot = (
                        ...
                        if not self.conic_fov
                            ...
                            agent_id, t_unit.pos, range="shoot_range", camp=camp
                        )
                    )
                    if can_shoot:
    ...
    def get_avail_actions(self, camp=0):
    ...
    def _kill_units(self, unit_tags):
        ...
        debug_commands = [debug_command[:self.n_agents], debug_command[self.n_agents:]]
        # self._controller.debug(debug_command)
        # self._parallel.run(
        #     (c.step, self._step_mul) for c in self._controllers
        # )
        self._parallel.run(
            (c.debug, debug_commands[i]) for i, c in enumerate(self._controllers)
        )
    ...
    def init_units(self, ally_team, enemy_team, episode_config={}):
        ...
        if ally_team and enemy_team:
            ...
            self._init_ally_unit_types(0, camp=self.camp)
            debug_cmd1 = self._create_new_team(ally_team, episode_config, ally=True)
            debug_cmd2 = self._create_new_team(enemy_team, episode_config, ally=False)
            debug_cmd = [debug_cmd1,debug_cmd2]
            self._parallel.run(
                (c.debug, debug_cmd[i]) for i, c in enumerate(self._controllers)
            )
            # self._controller.debug(debug_command)
            try:
                self._parallel.run(
                    (c.step, 1) for c in self._controllers
                )
        ...
        while True:
            ...
            for i in range(len(ally_units_sorted)):
                ...
                self.max_reward_2 += self.agents[i] .health_max + self.agents[i] .shield_max
                if self.debug:
                    logging.debug(
                        "Unit {} is {}, x = {}, y = {}".format(
                            len(self.agents),
            ...
            if self.camp == 1:
                ...
                for unit in self._obs.observation.raw_data.units:
                    if unit.owner == 2:
                        ...
                        if self._episode_count == 0:
                            #TODO: convert, this max reward only work for palyer 1
    ...
    def _init_ally_unit_types(self, min_unit_type, camp=0):
        ...
        if "10gen_" in self.map_name or "10sp_" in self.map_name or "mc_" in self.map_name:
            num_rl_units = 9
            self._min_unit_type = (
                len(self._controllers[0].data().units) - num_rl_units
            )
    ...
    def only_medivac_left(self, ally):
        ...
        if ally:
            ...
            medivac = self.medivac_id if self.camp == 1 else Terran.Medivac
            units_alive = [
                a
                for a in self.enemies.values()
                if (a.health > 0 and a.unit_type != medivac)
            ]
            if len(units_alive) == 0:
                return True
            return False
    ...
    def get_unit_health(self):
        health = []
        for a in self.agents.values():
            health.append(a.health)
        for a in self.enemies.values():
            health.append(a.health)
        return health

    def get_unit_by_id(self, a_id, camp=0):
        """Get unit by ID."""
        return self.agents[a_id] if camp == 0 else self.enemies[a_id]
