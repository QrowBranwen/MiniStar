...
class ReflectPositionDistribution(Distribution):
    ...
    def generate(self) -> Dict[str, Dict[str, Any]]:
        ...
        p = self.rng.random()
        if p > 0.5:
            ally_positions, enemy_positions = enemy_positions, ally_positions
        if self.n_enemies > self.n_units:
...
class SurroundedPositionDistribution(Distribution):
    ...
    def generate(self) -> Dict[str, Dict[str, Any]]:
        ...
        p = self.rng.random()
        if p > 0.5:
            self.n_enemies, self.n_units = self.n_units, self.n_enemies
        ...
        if p > self.p_threshold:
            ally_position, enemy_position = enemy_position, ally_position
        return {
