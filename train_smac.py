...
def make_train_env(all_args):
    def get_env_fn(rank):
        def init_env():
            ...
            elif all_args.env_name == "StarCraft2sp":
                from onpolicy.envs.starcraft2.SMACv2_sp import SMACv2
                env = SMACv2(capability_config=parse_smacv2_distribution(all_args), map_name=all_args.map_name)
...
def parse_args(args, parser):
    ...
    parser.add_argument("--camp", type=int, default=0)
...
def main(args):
    ...
    if all_args.share_policy:
        if "sp_" in all_args.map_name:
            config["num_agents"] = num_agents * 2
            from onpolicy.runner.shared.smac_runner_sp import SMACRunner as Runner
        else:
